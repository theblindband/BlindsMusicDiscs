...
public class SmallJukeboxBlockEntity extends BlockEntity implements SingleStackInventory.SingleStackBlockEntityInventory {
    ...
    public SmallJukeboxBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntityTypes.SMALL_JUKEBOX_BLOCK_ENTITY, pos, state);
    }
    ...
    private void onRecordStackChanged(boolean hasRecord) {
        if (this.world != null && this.world.getBlockState(this.getPos()) == this.getCachedState()) {
            this.world.setBlockState(this.getPos(), this.getCachedState().with(SmallJukeboxBlock.HAS_RECORD, Boolean.valueOf(hasRecord)), Block.NOTIFY_LISTENERS);
    ...
    public static void tick(World world, BlockPos pos, BlockState state, SmallJukeboxBlockEntity blockEntity) {
        blockEntity.getManager().tick(world, state);
    }
